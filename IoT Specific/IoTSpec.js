...
function runCommand() {
  if (count < maxCount) {
    ...
    const curlCommand = `curl -X POST "http://localhost:3000/data" -H "Content-Type: application/json" -d "{\\"roomNum\\":6, \\"waterValue\\":${waterValue}, \\"time\\":\\"${time}\\"}"`; // chose the room number here after "roomNum"  
